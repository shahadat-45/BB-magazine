<div class="max-w-6xl mx-auto mt-20 px-7 lg:px-0">
    <div class="mb-12 border-t border-gray-400 pt-8">
        ...
        <div class="flex justify-between items-center mb-8">
                <h2 class="text-lg md:text-2xl lg:text-3xl font-semibold uppercase">{{ "Article Categories" }}</h2>
                <a href="" class="text-sm text-gray-700 font-medium uppercase flex items-center">
                    ALL CATEGORIES
                    <svg class="w-4 h-4 ml-1 text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M17 8l4 4m0 0l-4 4m4-4H3" />
                    </svg>
                </a>{{-- {{ route('all-categories') }} --}}
        </div>

        <p class="text-gray-700 mb-6">
            {{ "Explore the vibrant beauty, culture, and spirit of Bangladesh through our curated gallery. Each image tells a story of tradition, innovation, and resilience." }}
        </p>
            ...
            @foreach ($articleCategories as $category)
                <div class="bg-white border border-gray-300 overflow-hidden p-3">
                    ...
                    <div class="p-3 text-center">
                        <a href="" title="{{ $category->name }}"{{-- {{ route('frontend.category', $category->slug) }} --}}
